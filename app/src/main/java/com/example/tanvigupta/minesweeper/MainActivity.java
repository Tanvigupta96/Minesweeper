    ...
    public void setUpBoard() {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                button.setOnLongClickListener((View.OnLongClickListener) this);
    ...
    public void setMines(int curRow,int curCol) {
        ...
        while (count < noOfMines) {
            ...
            if(randRow != curRow || randCol != curCol) {
                MButtons button = board[randRow][randCol];
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < n; j++) {
                MButtons button = board[i][j];

                if (button.value == -1) {

                    for (int k = 0; k < 8; k++) {
                        if (i + x[k] >= 0 && i + x[k] < m && j + y[k] >= 0 && j + y[k] < n) {
                            MButtons button1 = board[i + x[k]][j + y[k]];
                            if (!button1.hasMine()) {
        ...
        //if it is blank
            ...
            for (int i = 0; i < 8; i++) {
                if ((row + x[i] >= 0 && row + x[i] < m && col + y[i] >= 0 && col + y[i] < n)) {
                    MButtons btn = board[row + x[i]][col + y[i]];
